...
func CallWithToken(ctx context.Context, client *http.Client, errorMsg string, method string, urlStr string, postReq any, res any) (err error) {
	auth, err := interception.AuthFromContextCompatible(ctx)
	if err != nil {
		t, err := interception.BearerTokenFromContextCompatible(ctx)
		if err != nil {
			return err
		}
		auth = "Bearer " + t
	}
	return call(ctx, client, errorMsg, method, urlStr, postReq, res, auth)
}
