...
func (h *hydra) IntrospectV2(ctx context.Context, token string) (info Ihydra.TokenIntrospectInfo, err error) {
	target := fmt.Sprintf("%v/admin/oauth2/introspect", h.adminAddress)
	req, err := http.NewRequest(http.MethodPost, target, bytes.NewReader([]byte(fmt.Sprintf("token=%v", token))))
	if err != nil {
		log.WithContext(ctx).Error("IntrospectV2 NewRequest", zap.Error(err))
		return
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	resp, err := h.client.Do(req.WithContext(ctx))
	if err != nil {
		log.WithContext(ctx).Error("IntrospectV2 Post", zap.Error(err))
		return
	}
	...
	defer func() {
		closeErr := resp.Body.Close()
		if closeErr != nil {
			log.WithContext(ctx).Error("IntrospectV2 resp.Body.Close", zap.Error(closeErr))
		}
	}()
