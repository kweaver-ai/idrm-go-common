...
type DrivenUserMgnt interface {
	GetUserNameByUserID(ctx context.Context, userID string) (name string, isNormalUser bool, depInfos []*DepInfo, err error)
	...
	GetUserManagementAppList(ctx context.Context, args *AppListArgs) (result *base.PageResult[AppEntry], err error)
...
type UserManagementApp interface {
	GetUserManagementAppList(ctx context.Context, args *AppListArgs) (result *base.PageResult[AppEntry], err error)
	GetUserManagementApp(ctx context.Context, id string) (result *AppEntry, err error)
}
...
type UserInfo struct {
	...
	ParentDeps []*Parent         // 父部门信息
}
type Parent struct {
	Department []*Department //父部门信息
}
...
func NewDefaultAppListArgs() *AppListArgs {
	return &AppListArgs{
		Limit:  20,
		Offset: 0,
	}
}
func NewAllAppListArgs() *AppListArgs {
	return &AppListArgs{
		Limit:  1000,
		Offset: 0,
	}
