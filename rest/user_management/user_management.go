...
func NewUserMgnt(httpClient httpclient.HTTPClient, UserMgntUrl string) DrivenUserMgnt {
	usermgntOnce.Do(func() {
		...
		usermgnt = &usermgntSvc{
			baseURL:       fmt.Sprintf("http://%s/api/user-management", UserMgntUrl),
			publicBaseURL: fmt.Sprintf("%s/api/user-management", base.Service.UserMgnPublicHost),
...
func NewUserMgntByService(client *http.Client) DrivenUserMgnt {
	return NewUserMgnt(httpclient.NewMiddlewareHTTPClient(client), base.RemoveSchema(base.Service.UserMgnHost))
}
...
func (u *usermgntSvc) GetUserNameByUserID(ctx context.Context, userID string) (name string, isNormalUser bool, depInfos []*DepInfo, err error) {
...
func (u *usermgntSvc) GetUserInfoByID(ctx context.Context, userID string) (userInfo UserInfo, err error) {
	target := fmt.Sprintf("%s/v1/users/%s/%s", u.baseURL, userID, "groups,account,name,csf_level,frozen,email,telephone,third_attr,third_id,parent_deps")
